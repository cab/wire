...
//    }
    ...
    class EnumConstantNotFoundException(val value: Int, type: KClass<*>) : IllegalArgumentException("Unknown enum tag " + value + " for " + type.qualifiedName)

    private class MapProtoAdapter<K, V> internal constructor(keyAdapter: ProtoAdapter<K>, valueAdapter: ProtoAdapter<V>) : ProtoAdapter<Map<K, V>>(FieldEncoding.LENGTH_DELIMITED, Map::class) {
    ...
    private class MapEntryProtoAdapter<K, V> internal constructor(internal val keyAdapter: ProtoAdapter<K>, internal val valueAdapter: ProtoAdapter<V>) : ProtoAdapter<Map.Entry<K, V>>(FieldEncoding.LENGTH_DELIMITED, Map.Entry::class) {
    ...
    companion object {
        ...
        fun <M : Message<M, B>, B : Builder<M, B>> newMessageAdapter(type: KClass<M>): ProtoAdapter<M> {
            return RuntimeMessageAdapter.create(type)
        }

        /** Creates a new proto adapter for `type`.  */
        fun <E : WireEnum> newEnumAdapter(type: KClass<E>): EnumAdapter<E> {
        ...
        operator inline fun <reified M : Message<*, *>> get(message: M): ProtoAdapter<M> {
            return get(M::class) as ProtoAdapter<M>
        }

        /** Returns the adapter for `type`.  */
        operator fun <M : Any> get(type: KClass<M>): ProtoAdapter<M> {
//            try {
                return type.members.find { it.name == "ADAPTER" }!!.call(null) as ProtoAdapter<M>
...
//        }
        ...
        val BOOL: ProtoAdapter<Boolean> = object : ProtoAdapter<Boolean>(
            ...
            override fun decode(reader: ProtoReader): Boolean {
                ...
                throw IOException("Invalid boolean value ${value}")
            }
        }
        val INT32: ProtoAdapter<Int> = object : ProtoAdapter<Int>(
                FieldEncoding.VARINT, Int::class) {
        ...
        val FIXED32: ProtoAdapter<Int> = object : ProtoAdapter<Int>(
                FieldEncoding.FIXED32, Int::class) {
            override fun encodedSize(value: Int): Int {
        ...
        val FLOAT: ProtoAdapter<Float> = object : ProtoAdapter<Float>(
            ...
            override fun encode(writer: ProtoWriter, value: Float) {
                writer.writeFixed32(value.toRawBits())
            }
        ...
        val DOUBLE: ProtoAdapter<Double> = object : ProtoAdapter<Double>(
            ...
            override fun encode(writer: ProtoWriter, value: Double) {
                writer.writeFixed64(value.toRawBits())
            }
