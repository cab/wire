...
class GrpcClient private constructor(
          ...
          override fun invoke(proxy: Any, method: Method, args: Array<out Any>): Any? {
            return when (val grpcMethod = methodToService[method] as GrpcMethod<*, *>) {
              ...
              is GrpcMethod.StreamingRequest -> {
                val continuation = args[0] as Continuation<Any>
                continuation.invokeSuspending {
              ...
              is GrpcMethod.FullDuplex -> {
                val continuation = args[0] as Continuation<Any>
                continuation.invokeSuspending {
                  grpcMethod.invoke(grpcClient = this@GrpcClient)
                }
              }
            }
          }
  ...
  how do I do work on the IO thread and then resume when that's done
  ...
  internal fun <S, R> call(
    ...
    // READ A MESSAGE (non blocking, suspending code)
    // WRITE IT TO THE STREAM (slow blocking bad)
    // REPEAT + We have to wait for the writing to end before closing the writer

    // Stream messages from the request channel to the request body stream.
    CoroutineScope(context).launch(newSingleThreadContext("Writing Request")) {
      val requestWriter = GrpcWriter.get(requestBody.createSink(), grpcMethod.requestAdapter)
      requestWriter.use {
        for (message in requestChannel) {
          // TODO: when the dispatcher returns only then do we continue
